...
#name the simulation
folder_name = 'params_exploration_occupancy_LN+OFF_slow'
...
filepath = f"/user/sebert/home/Documents/Simulations/osr_model/explore_occupancy_params"
...
# range of k_rel 
k_ratios = np.round(np.arange(0.1,10,0.1),1)
k_rec = 1
# range of beta
betas = np.logspace(0,1,100) * 100
...
df = pandas.DataFrame(columns = ['k_rec', 'k_rel', 'k_ratio', 'beta', 'tau_n', 'slope', 'n_diff','n_diff_calc'])
print('step2')
print(k_ratios)
for k_ratio in k_ratios:
    ...
    for bet in betas: 
        ...
        k_rel = k_rec/k_ratio
        tau_n = 1/(k_rec+k_rel*bet)

        simulation_name = f'osr_k_ratio_{k_ratio}_beta_{bet}'
        ...
        n_calcs = calc_n_f(k_rec,k_rel,bet,frequencies)
...
# save the dataframe
df.to_csv(f'{filepath}/{folder_name}/DataFrame_occupancy_params_slope_effect.csv')
