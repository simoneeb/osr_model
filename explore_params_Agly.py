...
#name the simulation
folder_name = 'params_exploration_Agly_LN_slow'
...
#weights
wB_init = 50.0             # [1/s]
wA1_init = -50.0 # -60.0    #20.8   # [1/s]
wA2_init =  -50.0 # -80.0     #10.0  # [1/s]
...
# range of k_rel 
tau_A1s = np.arange(0.18,0.28,0.01)

weights = np.arange(0,80,1) * -1
# range of beta
# calculate analytically 


df = pandas.DataFrame(columns = ['w_A1', 'tau_A1' 'slope', 'delay_16Hz'])


for tau_A1 in tau_A1s:
    print(f'tau_A1 {tau_A1}')
    for w_A1 in weights: 
        print(f'w_A1 {w_A1}')
        

        simulation_name = f'osr_tau_A1_{tau_A1}_w_A1_{w_A1}'
        print(f'{simulation_name} simulation')
        filename = f"simulation_{simulation_name}"
        filepath_simulation = os.path.join(filepath_paramset,"simulations",f"{filename}")


        # TODO use Model.modify_params
        Model.modify_params('tau_A1',tau_A1)
        Model.modify_params('w_A1',w_A1)
        Model.modify_params('w_A2',wA2_init+w_A1)
...
# save the dataframe
df.to_csv(f'{filepath}/DataFrame_Agly_params_slope_effect.csv')
